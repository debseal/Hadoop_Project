...
int mpi_read(char *filename, int *numUrls, int ***am_index, int** adjacency_matrix, MPI_Comm comm) {
	...
	if (rank == 0) {
		...
		int result = file_read(filename, &total_num_urls, am_index, adjacency_matrix);
	...
	if (rank == 0) {
		...
		for (i = 1; i < nproc; i++) {
			...
			MPI_Send((*adjacency_matrix) + send_start_index, msg_size, MPI_INT, i, 1, comm);
		}
		...
		block_size = (rem > 0) ? divide + 1 : divide;
		msg_size = 0;
		for (i = 0; i < block_size; i++) msg_size += (*am_index)[i][1];

		(*adjacency_matrix) = realloc(*adjacency_matrix, msg_size * sizeof(int));
		assert(*adjacency_matrix != NULL);

		/* reduce the am_index[total_num_urls][2] to am_index[local_num_urls][2] */
		(*am_index)[0] = realloc((*am_index)[0], block_size * 2 * sizeof(int));
		assert((*am_index)[0] != NULL);
	} else {
		/* local numUrls */
		(*numUrls) = (rank < rem) ? divide + 1 : divide;

		//assert((*adjacency_matrix) != NULL); //TODO free?
		int recv_msg_size;
		MPI_Recv(&recv_msg_size, 1, MPI_INT, 0, 0, comm, &status);
		(*adjacency_matrix) = (int*) malloc(recv_msg_size * sizeof(int));

		MPI_Recv((*adjacency_matrix), recv_msg_size, MPI_INT, 0, 1, comm, &status);

		(*am_index) = (int**) malloc((*numUrls) * sizeof(int *));
		assert((*am_index) != NULL);
		(*am_index)[0] = (int*) malloc((*numUrls) * 2 * sizeof(int));
		assert((*am_index)[0] != NULL);
		for (i = 1; i < *numUrls; i++) (*am_index)[i] = (*am_index)[i - 1] + 2;

		MPI_Recv((*am_index)[0], (*numUrls) * 2, MPI_INT, 0, 2, comm, &status);
	}
	return 1;
}

int file_read(char *filename, int *total_num_urls, int ***am_index, int **adjacency_matrix) {
	...
	if (_debug) {
		...
		printf("File %s total_num_web_pages: %d\n", filename, total_num_web_pages);
	}

	/* allocate memory for adjacency_matrix[] and read all adjacency_matrix items */
	(*adjacency_matrix) = (int*) malloc(total_num_web_pages * sizeof(int));
	if ((*adjacency_matrix) == NULL) {
		fprintf(stderr, "Error: failed to allocate memory for adjacency_matrix\n");
		return -1;
	}
...
int mpi_write(char *filename, int totalNumUrls, double *rank_values_table, MPI_Comm comm) {
	...
	if (rank == 0) {
		printf("Proc:%d is writing rank values of %d urls to file pagerank.output\n", rank, totalNumUrls);
