...
public class SearchEngineTester {
	...
	public static void searchKeyword(String keyword) throws Exception {
        ...
        for (KeyValue kv : indexRow.list()) {
			...
			String pageDocId = Bytes.toString(kv.getQualifier());
	        ...
	        Get gData = new Get(pageBytes);
	        Result dataRow = dataTable.get(gData);
			
            String pageUri = Bytes.toString(dataRow.getValue(Constants.CF_DETAILS_BYTES,Constants.QUAL_URI_BYTES));
			
			// Step 3: get the page rank value of this page from clueWeb09PageRankTable
            Result prRow = prTable.get(gData);
			float pageRank=0;
			for (KeyValue pkv : prRow.list()){
